...
export const ExpenseScene3D: React.FC<ExpenseScene3DProps> = ({
  ...
  useEffect(() => {
    ...
    // 4. Studio Lighting System with Warm Aesthetic Accents
    const ambientLight = new THREE.AmbientLight(0xffffff, 1.1);
    scene.add(ambientLight);

    const dirLight1 = new THREE.DirectionalLight(0xD5EBDA, 1.3); // Sage Soft Light
    dirLight1.position.set(20, 30, 20);
    scene.add(dirLight1);

    const dirLight2 = new THREE.DirectionalLight(0xF3E4F1, 1.1); // Lavender Fill Light
    dirLight2.position.set(-20, 20, -20);
    scene.add(dirLight2);

    const centerPointLight = new THREE.PointLight(0xffffff, 0.9, 50);
    centerPointLight.position.set(0, 5, 0);
    scene.add(centerPointLight);

    // 5. Grid Pedestal Platform (Soft Warm Slate)
    const gridHelper = new THREE.GridHelper(30, 30, 0xCBD5E1, 0xE2E8F0);
    gridHelper.position.y = -3;
    scene.add(gridHelper);

    // Floating Ambient Dust Particles (Pastel Tinted)
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full h-[400px] sm:h-[480px] lg:h-[520px] rounded-2xl overflow-hidden bg-white border border-stone-200/80 shadow-card backdrop-blur-md select-none"
    >
      {/* 3D WebGL Canvas */}
      <canvas 
        ref={canvasRef} 
      ...
      <div className="absolute top-3.5 left-3.5 right-3.5 z-20 flex flex-wrap items-center justify-between gap-2 pointer-events-none">
        ...
        <div className="pointer-events-auto flex items-center bg-white/95 border border-stone-200/90 rounded-xl p-1 shadow-card backdrop-blur-md">
          <button
            type="button"
            onClick={(e) => { e.stopPropagation(); setVisualMode('rings'); }}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${
              visualMode === 'rings'
                ? 'bg-[#FAFAEB] text-stone-900 shadow-xs border border-stone-200'
        ...
        {/* Total Expense Pill */}
        <div className="bg-white/95 border border-stone-200/90 rounded-xl px-3.5 py-1.5 shadow-card backdrop-blur-md flex items-center gap-2.5">
          <span className="w-2.5 h-2.5 rounded-full bg-[#D5EBDA] border border-[#8CCA9A]" />
          <div className="flex items-baseline gap-1.5">
            <span className="text-[11px] font-medium text-stone-500 uppercase tracking-wider">Volume</span>
            <span className="text-xs font-bold text-stone-900">{formatCurrency(totalExpense, currency)}</span>
          </div>
        </div>
      </div>

      {/* Right Floating Camera Controls & Tools */}
      <div className="absolute right-3.5 top-16 z-20 flex flex-col gap-1.5 pointer-events-auto">
        <button
          type="button"
          onClick={(e) => { e.stopPropagation(); setAutoRotate(!autoRotate); }}
          title={autoRotate ? "Pause Orbit" : "Resume Orbit"}
          className={`p-2 rounded-xl border backdrop-blur-md text-xs font-medium transition-all cursor-pointer ${
            autoRotate 
              ? 'bg-[#D5EBDA] border-[#8CCA9A] text-stone-900 shadow-xs' 
              : 'bg-white/90 border-stone-200 text-stone-600 hover:text-stone-900 hover:bg-[#FAFAEB]'
          }`}
        >
        ...
        <button
          ...
          className={`p-2 rounded-xl border backdrop-blur-md text-xs font-medium transition-all cursor-pointer ${
            wireframeMode 
              ? 'bg-[#F3E4F1] border-[#E6C6E1] text-purple-950 shadow-xs' 
              : 'bg-white/90 border-stone-200 text-stone-600 hover:text-stone-900 hover:bg-[#FAFAEB]'
          }`}
        >
        ...
        <button
          ...
          className="p-2 rounded-xl bg-white/90 border border-stone-200 text-stone-600 hover:text-stone-900 hover:bg-[#FAFAEB] backdrop-blur-md transition-all text-xs cursor-pointer shadow-xs"
        >
        ...
        {onCaptureSnapshot && (
          <button
            ...
            title="Snapshot"
            className="p-2 rounded-xl bg-[#FAFAEB] hover:bg-[#F4DACD] border border-stone-300 text-stone-800 transition-all cursor-pointer shadow-xs"
          >
      ...
      {hoveredData && (
        <div
          className="absolute pointer-events-none z-30 transform -translate-x-1/2 -translate-y-full mb-2 px-3.5 py-2.5 rounded-xl bg-white border border-stone-200 shadow-xl backdrop-blur-md animate-fade-in"
          style={{
        ...
        >
          <div className="flex items-center gap-1.5 mb-1">
            ...
            />
            <span className="text-xs font-semibold text-stone-900">{hoveredData.name}</span>
          </div>
      ...
      <div className="absolute bottom-3 left-3.5 right-3.5 flex items-center justify-between text-[11px] text-stone-500 pointer-events-none">
        <div className="flex items-center gap-2 bg-white/95 px-3 py-1 rounded-full border border-stone-200/90 shadow-xs backdrop-blur-sm">
          <span>Click & Drag to Orbit</span>
          <span className="hidden sm:inline">•</span>
          <span className="hidden sm:inline">Scroll to Zoom</span>
          <span className="hidden sm:inline">•</span>
          <span className="hidden sm:inline">Click Slices to Filter</span>
        </div>
        {selectedCategory && (
          <button
            onClick={() => onSelectCategory && onSelectCategory(null)}
            className="pointer-events-auto bg-[#D5EBDA] hover:bg-[#B2DCBC] text-stone-900 border border-[#8CCA9A] font-medium px-3 py-1 rounded-full text-xs transition-all shadow-xs"
          >
            Clear Filter ✕
          </button>
        )}
